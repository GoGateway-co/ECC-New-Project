...
Copy-paste prompt to add the `/sync-ecc` command to any project that already has ECC installed.

---

## Prompt (copy everything below this line into a new Claude Code session)

Create the file `commands/sync-ecc.md` with the following exact content:
...
After creating the file, confirm it exists and report the current ECC version from `.claude-plugin/plugin.json`.
